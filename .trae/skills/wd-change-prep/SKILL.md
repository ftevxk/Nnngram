...
- 代码上下文：
  ...
  - 项目工具：新增改动功能优先查找项目中可复用的功能工具
  - 工具技能：通过Find Skills查找适用的工具技能
...
**失败回滚策略**
- 若入口不清晰：先写最小观测（受 `BuildVars.LOGS_ENABLED` 控制，使用 `FileLog.d/e`，日志以 `"wd "` 开头、中文描述且不含敏感信息）来确认真实路径，再继续。
...
- 若会新增日志：预先定义日志点位与脱敏策略（使用 `FileLog.d/e`，日志以 `"wd "` 开头）
