...
# TF-IDF 特征提取（广告模型使用独立的vectorizer）
...
# 保存广告模型的 vectorizer 参数（用于 Android 端）
# 保存词汇表
vocab_ad = ad_vectorizer.vocabulary_
with open('ad_vocabulary.txt', 'w', encoding='utf-8') as f:
    for word, idx in sorted(vocab_ad.items(), key=lambda x: x[1]):
        f.write(f"{word}\n")

# 保存 IDF 值
idf_ad = ad_vectorizer.idf_
with open('ad_idf.txt', 'w') as f:
    for val in idf_ad:
        f.write(f"{val}\n")

# 保存配置
config_ad = {
    'max_features': ad_vectorizer.max_features,
    'ngram_range': list(ad_vectorizer.ngram_range),
    'vocab_size': len(vocab_ad)
}
with open('ad_config.json', 'w') as f:
    json.dump(config_ad, f, indent=2)

print("广告分类模型训练完成！")
print(f"词汇表大小: {len(vocab_ad)}")
print(f"配置: {config_ad}")
...
files.download('ad_vocabulary.txt')
files.download('ad_idf.txt')
files.download('ad_config.json')

print("所有文件已下载！")
print("\n部署到 Android 项目的文件列表：")
print("TMessagesProj/src/main/assets/ai_ad_filter/")
print("├── model.tflite          # 广告分类模型")
print("├── labels.txt            # 广告标签")
print("├── ad_vocabulary.txt     # 广告模型词汇表")
print("├── ad_idf.txt           # 广告模型 IDF 值")
...
```csv
text,label,topic,intent
"专业上分服务，安全可靠，微信：bet888",ad,transaction,offer
"你好，最近怎么样？",normal,chat,inform
```
