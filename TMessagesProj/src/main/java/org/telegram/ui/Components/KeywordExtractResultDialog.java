...
public class KeywordExtractResultDialog extends AlertDialog {
    ...
    private static class KeywordCheckCell extends FrameLayout {
        ...
        public KeywordCheckCell(Context context, ExtractedKeywordItem item) {
            ...
            addView(checkBox, LayoutHelper.createFrame(24, 24, Gravity.CENTER_VERTICAL | Gravity.START));

            // 关键词文本
            keywordText = new TextView(context);
            keywordText.setText(item.keyword);
            keywordText.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 16);
            keywordText.setTextColor(Theme.getColor(Theme.key_dialogTextBlack));
            keywordText.setMaxLines(1);
            keywordText.setEllipsize(TextUtils.TruncateAt.END);
            addView(keywordText, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT,
                Gravity.CENTER_VERTICAL, 36, 0, 80, 0));

            // 权重文本
            weightText = new TextView(context);
            String weightStr = String.format("%.2f", item.weight);
            weightText.setText("权重:" + weightStr);
            weightText.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 12);
            weightText.setTextColor(Theme.getColor(Theme.key_dialogTextGray3));
            addView(weightText, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT,
